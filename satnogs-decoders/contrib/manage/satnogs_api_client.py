...
def satnogs_api_get(url, api_token):
    try:
        r = requests.get(url=url, headers={'Authorization': 'Token ' + api_token})
        r.raise_for_status()
...
def fetch_telemetry(norad_id, max_frames, url, api_token):
    ...
    response = satnogs_api_get(url, api_token)
    ...
    if next_page_available and len(frames) < max_frames:
        parts = response.headers['Link'].split(',')
        for part in parts:
            if part[-5:-1] == 'next':
                next_page_url = part[1:-13]

    while next_page_available and (not max_frames or len(frames) < max_frames):
        response = satnogs_api_get(url, api_token)
        frames.extend(response.json())
    ...
    # We might have fetched more then the required number of frames.
    # Discard the surplus
